...
if __name__ == '__main__':
    ...
    for m, n in [(8, 12)]:
        ...
        # 50 graphs
        for _g in range(50):
            f = np.random.randint(10, 25, m).tolist()
            h = np.round(np.random.uniform(0.1, 0.2, m), 3).tolist()
            _num_arcs = int((m+n) * 1.2)  # 3 <-- 2018 ShuJia POMS; PS. int has the same function as floor in this case
